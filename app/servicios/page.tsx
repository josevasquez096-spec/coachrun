...
export default function Servicios() {
  ...
  return (
    <main className="shell">
      <Cabecera titulo={t('servicios.titulo')} nombre={perfil?.full_name} avatar={perfil?.avatar_url}
        frase={t('servicios.frase')} />
      ...
      <div className="servicio">
        <h3>{t('serv.pacingTitulo')}</h3>
        <p>{t('serv.pacingDesc')}</p>
        <ul>
          <li>{t('serv.pacing1')}</li>
          <li>{t('serv.pacing2')}</li>
          <li>{t('serv.pacing3')}</li>
        </ul>
      </div>

      <div className="servicio">
        <h3>{t('serv.unoTitulo')}</h3>
        <p>{t('serv.unoDesc')}</p>
        <ul>
          <li>{t('serv.uno1')}</li>
          <li>{t('serv.uno2')}</li>
      ...
      <div className="servicio">
        <h3>{t('serv.precioTitulo')}</h3>
        <p>{t('serv.precioDesc')}</p>
      </div>

      <div style={{ marginTop: 16, display: 'grid', gap: 10 }}>
        <a className="btn-wa" href={waLink(t('serv.saludoWa'))} target="_blank" rel="noopener noreferrer">
        ...
        <a className="btn ghost block" href={`mailto:${CORREO}?subject=${encodeURIComponent(t('serv.asuntoCorreo'))}`}>
          {t('serv.correo')}
        </a>
